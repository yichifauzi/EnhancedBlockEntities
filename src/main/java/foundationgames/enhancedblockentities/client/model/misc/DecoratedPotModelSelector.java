...
public class DecoratedPotModelSelector extends ModelSelector {
    ...
    private int getPatternIndex(Item sherd, int max) {
        return MathHelper.clamp(this.potteryPatterns.indexOf(patternOf(sherd)), 0, max - 1);
    }

    public static RegistryKey<String> patternOf(Item item) {
        RegistryKey<String> registryKey = DecoratedPotPatterns.fromSherd(item);
        if (registryKey == null) {
            return DecoratedPotPatterns.DECORATED_POT_SIDE_KEY;
        }
        return registryKey;
    }
}
